...
def _ensure_configured(config_path: str | None = None) -> None:
    ...
    if needs_onboard(config_path):
        print("[Jarvis] No LLM provider configured. Starting setup wizard...\n")
...
def _run_foreground(args) -> None:
    ...
    try:
        ...
        print(f"[Jarvis] Warning: LLM provider not configured ({exc})")
    ...
    if ch_to_start:
        ...
        print(f"[Jarvis] Channels ({label}): {', '.join(ch_to_start)}")
    ...
    print(f"[Jarvis] Web dashboard: http://localhost:{port}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pythonclaw",
        description="Jarvis — Autonomous AI Agent Framework",

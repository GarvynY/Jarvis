"""
Daemon process lifecycle manager for Jarvis.
...
def start_daemon(
    ...
    """Start Jarvis as a background daemon.
    ...
    log_handle.write(f"\n{'='*60}\n")
    log_handle.write(f"Starting Jarvis daemon at {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    if proc.poll() is not None:
        print(f"[Jarvis] Daemon failed to start (exit code {proc.returncode}).")
        print(f"[Jarvis] Check logs: {LOG_FILE}")
        _cleanup_pid()
        return

    print(f"[Jarvis] Daemon started (PID {proc.pid}).")
    print(f"[Jarvis] Dashboard: http://localhost:{port}")
    print(f"[Jarvis] Logs: {LOG_FILE}")
...
def stop_daemon() -> None:
    ...
    pid = read_pid()
    if not pid:
        print("[Jarvis] No daemon is running.")
        return

    if not _is_alive(pid):
        print(f"[Jarvis] Stale PID file (process {pid} not found). Cleaning up.")
        _cleanup_pid()
        return

    print(f"[Jarvis] Stopping daemon (PID {pid})...", end=" ", flush=True)
    try:

...
Priority order:
  1. ANTHROPIC_API_KEY env var  + anthropic package  → Claude Haiku
  2. DEEPSEEK_API_KEY env var   + openai package      → DeepSeek Chat
  3. pythonclaw.json deepseek.apiKey                 → DeepSeek Chat (config fallback)
...
def call_llm(
    ...
    Tries Anthropic first, then DeepSeek, then returns ("", {}).

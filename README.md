...
- **多 Agent 研究工作流**：将研究任务拆解为 FX、News、Macro、Risk 等无状态专家 Agent，再由 Supervisor 汇总。
- **证据优先的研究输出**：Agent 输出 evidence、confidence、source、missing_data，避免只给不可审计的结论。
...
在 Phase 9 中，Jarvis 引入 preset-driven 的并行研究工作流。CNY/AUD 不再是写死在系统里的唯一主题，而是第一个 research preset。每个研究任务会被拆解为多个无状态专家 Agent，例如 FX Agent、News Agent、Macro Agent 和 Risk Agent。
...
为了应对金融行研场景中的长上下文和多来源信息问题，Jarvis 正在引入 Runtime Micro-RAG / Dynamic Evidence Store。系统不再让上游 Agent 把长篇原文和分析全文传给 Supervisor，而是将证据切块并写入带强元数据的 Evidence Store，只向下游传递 chunk_id、summary、category、entity、importance 等引用信息。
...
Jarvis/
  ...
  pythonclaw/
    ...
    templates/skills/data/fx_monitor/       CNY/AUD 汇率监控与研究技能
      ...
      news_monitor.py
      research/
        agents/                             FX / News / Macro / Risk Agent
        coordinator.py
        supervisor.py
        runner.py
        schema.py
